...
export default async function VehicleDetailPage({ params }: PageProps) {
  ...
  const keyFeatures = [
    ...
    `Battery options: ${buyingSpecs.batterySpec?.value ?? "Source review pending"}`,
    buyingSpecs.variants.length > 1 ? `${buyingSpecs.variants.length} listed configurations in PlugV` : buyingSpecs.variants.length === 1 ? "One listed configuration in PlugV" : "Variant-level specifications are being verified",
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-slate-950 text-white">
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-6 px-4 pb-4 sm:px-6 lg:px-8">
        {buyingSpecs.batterySpec ? <div className="mb-4 text-sm leading-6 text-slate-300"><a className="text-sky-300 underline" href={buyingSpecs.batterySpec.sourceUrl}>Battery specification source</a><p>Source checked {buyingSpecs.batterySpec.checkedAt}. {buyingSpecs.batterySpec.scope}</p></div> : null}
        <VehicleVariantExplorer vehicleName={`${vehicle.brand} ${vehicle.name}`} bodyType={vehicle.type} seating={buyingSpecs.seats} listedRange={vehicle.range ?? "Awaiting official specification"} listedBattery={buyingSpecs.batterySpec?.value ?? "Source review pending"} variants={buyingSpecs.variantDetails} />
