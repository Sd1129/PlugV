...
const prompts = [
  ...
  "Can Tiago EV 24 kWh cover 180 km starting at 90%?",
];

function RecommendationCards({ reply }: { reply: SmartAssistantReply }) {
  if (!reply.recommendations?.length) return null;
  return <div className="mt-4 grid gap-3 sm:grid-cols-3">{reply.recommendations.map(({ vehicle, reasons }) => <Link key={vehicle.slug} href={`${"launch" in vehicle ? "/upcoming" : "/vehicles"}/${vehicle.slug}`} className="rounded-xl border border-white/10 bg-slate-950/70 p-3 transition hover:border-sky-300/30">
    <p className="text-sm font-semibold text-white">{vehicle.brand} {vehicle.name}</p>
    <p className="mt-1 text-xs text-sky-200">Catalogue candidate</p>
...
export default function AssistantPage() {
  ...
  function submit(text = input) {
    ...
    if (!value) return;
    const previousPrompt = [...messages].reverse().find((message) => message.role === "user")?.content;
    const reply = answerWithPlugV(value, previousPrompt);
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
          <aside>
            ...
            <p className="mt-5 text-base leading-8 text-slate-300">A free catalogue assistant for EV lookups, comparisons and clearly labelled trip estimates. Price and specification reviews are ongoing.</p>
