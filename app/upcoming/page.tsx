      ...
      <section className="relative isolate overflow-hidden border-b border-white/10">
        ...
        <div className="mx-auto grid w-full max-w-7xl gap-10 px-4 py-16 sm:px-6 sm:py-20 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-24">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-6 text-4xl font-semibold leading-[1.02] tracking-tight sm:text-5xl lg:text-7xl">What&apos;s next in electric mobility.</h1>
            <p className="mt-6 max-w-2xl text-base leading-8 text-slate-300 sm:text-lg">A carefully verified view of future EVs and official concepts relevant to Indian buyers—without presenting speculation as a confirmed launch.</p>
...
function UpcomingCard({ vehicle }: { vehicle: UpcomingVehicle }) {
  return <article className="group overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.04] shadow-[0_24px_80px_-30px_rgba(0,0,0,0.8)] transition hover:-translate-y-1 hover:border-sky-300/25">
    <div className={`relative h-52 bg-gradient-to-br ${accentFor(`${vehicle.brand}-${vehicle.name}`)}`}><div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.2),transparent_40%)]" /><div className="absolute left-5 top-5"><StatusBadge status={vehicle.status} /></div><div className="absolute inset-x-5 bottom-5"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-sky-200">{vehicle.brand}</p><h3 className="mt-2 text-3xl font-semibold">{vehicle.name}</h3><p className="mt-1 text-sm text-slate-300">{vehicle.segment}</p></div></div>
