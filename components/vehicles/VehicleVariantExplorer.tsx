  ...
  return <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.04]">
    ...
    {variants.length > 1 ? <div className="border-t border-white/10 bg-slate-950/25 p-5 sm:p-7">
      ...
      {selectedName === compareName ? <p className="mt-4 rounded-2xl border border-amber-300/15 bg-amber-400/[0.06] p-4 text-sm text-amber-100">Choose two different variants to compare.</p> : <><div className="mt-5 space-y-3 sm:hidden">
        {rows.map((row) => <article key={row.label} className="rounded-2xl border border-white/10 bg-slate-950/45 p-4"><h4 className="text-sm font-semibold text-slate-300">{row.label}</h4><dl className="mt-3 grid grid-cols-2 gap-3"><div><dt className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">{selectedName}</dt><dd className="mt-1 text-xs leading-5 text-slate-200">{valueFor(selected, row.key, row.key === "range" ? listedRange : row.key === "battery" ? listedPower : undefined)}</dd></div><div className="border-l border-white/10 pl-3"><dt className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">{compareName}</dt><dd className="mt-1 text-xs leading-5 text-slate-200">{valueFor(compared, row.key, row.key === "range" ? listedRange : row.key === "battery" ? listedPower : undefined)}</dd></div></dl></article>)}
